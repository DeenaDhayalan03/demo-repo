...
class ImagePullRequest(BaseModel):
    username: Optional[str] = Field(None, title="Username", description="Username for Docker registry authentication (only for private images).")
    password: Optional[str] = Field(None, title="Password", description="Password for Docker registry authentication (only for private images).")
    registry_url: Optional[str] = Field(None, title="Docker Registry URL", description="URL of the Docker registry (only for private images).")
    image_name: str = Field(..., title="Image Name (including tag)", description="Full name of the image to pull (e.g., deena12/dockerapi:latest)")
